...
async function submitStorageAccountRequest() {
    ...
    try {
        ...
        const workflowInputs = {
            ...
            kind: formData.get('kind') || 'StorageV2',
            sku: formData.get('sku') || 'Standard_LRS',
            accessTier: formData.get('accessTier') || 'Hot',
            enableHttpsTrafficOnly: document.getElementById('enableHttpsTrafficOnly')?.checked ? 'true' : 'false',
            minimumTlsVersion: formData.get('minimumTlsVersion') || 'TLS1_2',
            allowBlobPublicAccess: document.getElementById('allowBlobPublicAccess')?.checked ? 'true' : 'false',
